...
class ClockifyTocker:
    ...
    def _get_user(self):
        return requests.get('https://api.clockify.me/api/v1/user', headers={
            'Content-Type': 'application/json',
            'X-Api-Key': self.api_key,
        }).json()

    def get_all_projects(self):
        return requests.get(f'https://api.clockify.me/api/v1/workspaces/{self.workspace_id}/projects/', headers={
            'Content-Type': 'application/json',
            'X-Api-Key': self.api_key,
        }).json()
    ...
    def _stop_time_entry(self, user_id):
        ...
        result = requests.patch(f'https://api.clockify.me/api/v1/workspaces/{self.workspace_id}/user/{user_id}/time-entries/', headers={
            'Content-Type': 'application/json',
            'X-Api-Key': self.api_key,
        }, json={
            'end': time
    ...
    def _get_active_time_entry(self, user_id):
        entry = requests.get(f'https://api.clockify.me/api/v1/workspaces/{self.workspace_id}/user/{user_id}/time-entries?in-progress=true', headers={
            'Content-Type': 'application/json',
            'X-Api-Key': self.api_key,
        }).json()
        if entry:
            if 'timeInterval' in entry[0]:
                when = datetime.datetime.fromisoformat(entry[0]['timeInterval']['start'].replace('T', ' ')[:-1])
                if not self.when or when > self.when or self.active_entry == entry[0]['id']:
                    self.when = when
    ...
    def _set_active_project_id(self, active_project_id):
        ...
        self.active_project = {v: k for k, v in self.project_ids.items()}.get(self.active_project_id, None)

    def start_time_entry(self, detail, project_id):
        self.when = datetime.datetime.utcnow()
        self._set_active_project_id(project_id)
        time = self.when.isoformat()[:-7] + 'Z'
        entry = requests.post(f'https://api.clockify.me/api/v1/workspaces/{self.workspace_id}/time-entries/', headers={
            'Content-Type': 'application/json',
            'X-Api-Key': self.api_key,
        }, json={
            'description': detail,
