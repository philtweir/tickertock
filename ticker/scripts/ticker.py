...
def init(clockify_api_key, clockify_workspace_id):
    ...
    projects = [
        ...
        (p.get('memberships', False) and p['memberships'][0].get('membershipStatus', False) == 'ACTIVE') and
        not p.get('archived', True)
    ]
    print(projects)
    project_toml = {
        "projects": {p["name"]: {"name": p["name"], "colour": p["color"].replace("#", "")} for p in projects},
        "page": {"entries": [p["name"] for p in projects]}
    }
    with open(CONFIG_DIR / "projects.toml", "w") as f:
        toml.dump(project_toml, f)
        logging.info(f"Projects list written out to {CONFIG_DIR / 'project.toml'} - look here to start building!")
